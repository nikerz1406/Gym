 ...
 * @author Jason
 */
public class BranchViewPanel extends javax.swing.JPanel {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                for (int i = 0; i < tbView.getColumnCount(); i++) {
                    switch (tbView.getColumnName(i)) {
                        case "ID":
                            BranchMainPanel.selectId = (int) tableModel.getValueAt(row, i);
                            break;
                        case "Name":
                            selectName = tableModel.getValueAt(row, i).toString();
                            break;
                        case "Address":
                            selectAddress = tableModel.getValueAt(row, i).toString();
                            break;
                        case "Phone":
                            selectPhone = tableModel.getValueAt(row, i).toString();
                            break;
                    }
                }
                //Load data of the selectde row
                if(BranchMainPanel.selectId != -1){
                    String detailContent = "Branch :  " + selectName.toUpperCase() + "\n";
                    detailContent += "Address : " + selectAddress + "\n";
                    detailContent += "PhoneNo : " + selectPhone;
                    taDetail.setText(detailContent);
                    BranchMainPanel.btEdit.setEnabled(true);
                    BranchMainPanel.btDelete.setEnabled(true);
                }
            }
        });
    }
    ...
    private void fillTable(){
        BranchMainPanel mainPanel = new BranchMainPanel();
        mainPanel.getArrayList();
        DefaultTableModel tableModel = (DefaultTableModel) tbView.getModel();
        tableModel.setRowCount(0);
        filterTable(tableModel, BranchMainPanel.branchList);
